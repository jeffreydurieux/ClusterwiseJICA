# Wed Mar 16 13:38:44 2022
# Author: Jeffrey Durieux, MSc


# what: new simulation with overlap and correlation structure cjica --> ILS version
...
ILS_CJICA <- function(X, k, nc, scale = TRUE, iter, stepsize, titlevec, rational){
  ...
  while(it < iter & temperature < n){
    ...
    repeat{
      ...
      plot(losstrack, main = round(adjustedRandIndex(titlevec, newp), digits = 4))
...
Q <- c(2,4,6)
R <- c(3, 5)
N <- c(50, 75) ### do N = 100 separately using seed + 6840
rho <- c(0, .60, .94)
lap <- c(0,1,2)
Err <- c(.2, .4, .60)
rep <- 1:20
...
grid <- grid[rows,]
which(grid$Q==6 & grid$R==5 & grid$N==50& grid$Err==0.2& grid$rep==1 & grid$rho==0 & grid$lap==0)
sim = 6
...
for(sim in 1:nrow(grid)){
  ...
  cjica <- ILS_CJICA(X = simdata$Xe, k = grid[sim,]$R,
                     nc = grid[sim,]$Q, scale = T, iter = 1000, 
                     stepsize = 2,titlevec = simdata$P, rational = simdata$P)
  cjica$pvault[, which.min(cjica$lossvault)] %>% adjustedRandIndex(simdata$P)
  time <- proc.time() - ptm
